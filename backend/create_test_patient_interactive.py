#!/usr/bin/env python3
"""
Script interactivo para crear un paciente de prueba con datos faltantes.
Solicita las credenciales de PostgreSQL si no están en .env o si fallan.
"""
...
def get_db_credentials():
    """Obtiene las credenciales de la base de datos desde .env o las solicita interactivamente."""
    ...
    # Si no hay password en .env, solicitarlo
    if not DB_PASSWORD:
        print("\n🔐 No se encontró DB_PASSWORD en .env")
        print("   Por favor, ingresa las credenciales de PostgreSQL:")
        DB_USER = input(f"   Usuario [{DB_USER}]: ").strip() or DB_USER
        DB_PASSWORD = getpass.getpass("   Contraseña: ")
        DB_HOST = input(f"   Host [{DB_HOST}]: ").strip() or DB_HOST
        DB_PORT = input(f"   Puerto [{DB_PORT}]: ").strip() or DB_PORT
        DB_NAME = input(f"   Base de datos [{DB_NAME}]: ").strip() or DB_NAME
    
    return DB_HOST, DB_PORT, DB_NAME, DB_USER, DB_PASSWORD

def test_connection(DB_HOST, DB_PORT, DB_NAME, DB_USER, DB_PASSWORD):
...
def create_test_patient():
    ...
    for attempt in range(max_attempts):
        print(f"\n🔌 Intentando conectar a PostgreSQL (intento {attempt + 1}/{max_attempts})...")
        print(f"   Host: {DB_HOST}")
        print(f"   Port: {DB_PORT}")
        print(f"   Database: {DB_NAME}")
        print(f"   User: {DB_USER}")
        
        if test_connection(DB_HOST, DB_PORT, DB_NAME, DB_USER, DB_PASSWORD):
            print("✅ Conexión exitosa!")
            break
        else:
            if attempt < max_attempts - 1:
                print("❌ Error de autenticación. Por favor, verifica las credenciales:")
                DB_USER = input(f"   Usuario [{DB_USER}]: ").strip() or DB_USER
                DB_PASSWORD = getpass.getpass("   Contraseña: ")
            else:
